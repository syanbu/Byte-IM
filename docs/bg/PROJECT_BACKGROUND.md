...
mock server 使用 Java/Netty/Maven，承担本地账号注册登录、HTTP 鉴权、WebSocket 鉴权、消息转发、ACK、离线暂存、持久化恢复、friends/profile/group/group-read/push/OSS upload-target 等本地验证职责。服务端是辅助验证环境，不是生产后端。
...
- B4 本地 SQLite 历史消息分页；服务端历史查询尚未接入。
...
- B10 群聊基础能力，包括 server-backed group creation、群消息收发、群成员存储、群资料页、已加入群列表、@ mention 元数据、@ 输入/高亮和 @ 我未读计数；成员增删/退群等完整群管理仍未产品化。
- B11 图片消息首版，包括图库选择、多选拆成多条消息、OSS upload-target、上传/发送失败分层、缩略图缓存和渐进式显示。
- B12 消息撤回和已读回执首版，包括单聊已读回执、群聊已读人数/读者列表、撤回状态持久化、聊天 UI 撤回展示、会话预览更新和离线撤回通知补发。
- B13 mock 推送首版，包括 mock push token 注册、服务端 pending push 入队、Android WorkManager 轮询、通知展示、ack 和点击 deep-link 进入会话；真实厂商推送不在当前范围。
- Contacts/Friends 能力，包括服务端好友列表、Android 本地 `friend_contacts` 缓存、联系人页、联系人资料页，以及创建群聊时从联系人中选择成员。
...
- 需求目标：[`PROJECT_TARGET.md`](PROJECT_TARGET.md)
