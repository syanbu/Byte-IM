...
- Android silently refreshes when the access token is expired but the refresh token is valid.
...
- Refresh token hashes are persisted in SQLite and marked revoked on logout.
...
| 2026-05-22 | B1 Dual Token Auth | `mvn -q test` in `mock-server` | Passed: login/register issue access and refresh tokens, refresh returns a new access token, logout revokes refresh tokens, and WebSocket AUTH rejects invalid or expired access tokens. |
