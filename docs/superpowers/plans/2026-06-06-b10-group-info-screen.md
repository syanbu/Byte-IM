...
- 群聊信息页（`GroupInfoScreen`）为一个新的全屏路由，承载两段内容：
  ...
  2. **群聊名称卡片**（在网格下方）：复用 `ByteImListSurface` 的卡片样式，仿照 `MeScreen.kt:557-586` 的 `ProfileNameRow` 形态 —— 左侧"群聊名称"标签（weight 0.35f），右侧群名（weight 0.65f，`TextAlign.End`，`TextSecondary`），右侧加 `ChevronRightIcon`。点击 → 弹起 `AlertDialog`（复用 [ChatScreen.kt:387-417](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L387-L417) 的形态），保存后调 `groupRepository.renameGroup(...)`。
...
| 用途 | 路径 | 备注 |
|---|---|---|
| `AvatarImage(avatarUrl, displayName, modifier)` | [ui/AvatarImage.kt](../../app/src/main/java/com/buyansong/im/ui/AvatarImage.kt) | 已支持 `isGroup = true` 时的"群"字占位 |
| `ByteImTopBar` / `ByteImListSurface` / `ByteImDimensions` / `ByteImColors` | [ui/ByteImUi.kt](../../app/src/main/java/com/buyansong/im/ui/ByteImUi.kt) | 直接复用，不新增 token |
| `GroupInfo` / `GroupMember` / `GroupMemberRole` | [storage/StorageModels.kt:70-87,108-111](../../app/src/main/java/com/buyansong/im/storage/StorageModels.kt#L70) | 数据模型已存在 |
| `GroupRepository.syncMembers(accessToken, groupId)` | [group/GroupRepository.kt:80-89](../../app/src/main/java/com/buyansong/im/group/GroupRepository.kt#L80) | 一次调用同时刷新群元信息 + 成员列表到本地 |
| `GroupRepository.localMembers(groupId)` | [group/GroupRepository.kt:91](../../app/src/main/java/com/buyansong/im/group/GroupRepository.kt#L91) | 缓存读取，UI 首屏立即可见 |
| `GroupRepository.renameGroup(accessToken, groupId, name)` | [group/GroupRepository.kt:56-68](../../app/src/main/java/com/buyansong/im/group/GroupRepository.kt#L56) | 已做 trim/空名校验、持久化 `GroupInfo`、更新 `Conversation.peerName/title/avatarUrl` |
| 网格容器参考（列数改 5） | [chat/AlbumPickerScreen.kt:78-91](../../app/src/main/java/com/buyansong/im/chat/AlbumPickerScreen.kt#L78) | 唯一一个 `LazyVerticalGrid` |
| `AlertDialog` 改名模式 | [chat/ChatScreen.kt:387-417](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L387) | 直接复用 text/confirmButton/dismissButton 结构 |
| `ContactProfileScreen` 入口路由与回调风格 | [contacts/ContactProfileScreen.kt:40-79](../../app/src/main/java/com/buyansong/im/contacts/ContactProfileScreen.kt#L40) | 新页面的全屏骨架可一一对应 |
| `MeScreen.kt:557-586` 的 `ProfileNameRow` 卡片行 | [profile/MeScreen.kt:557-586](../../app/src/main/java/com/buyansong/im/profile/MeScreen.kt#L557) | "群聊名称"行直接照抄样式 |
| 字符串集中管理范例 | [profile/MeDisplayPolicy.kt](../../app/src/main/java/com/buyansong/im/profile/MeDisplayPolicy.kt) | 新建 `GroupInfoDisplayPolicy.kt` 时与之并列 |
| 顶部后按钮优先级链范例 | [profile/MeBackPolicy.kt](../../app/src/main/java/com/buyansong/im/profile/MeBackPolicy.kt) | `GroupInfoScreen` 简单，只需单层 back，无需新建 policy |

## 改动清单

### 新增

1. **`app/src/main/java/com/buyansong/im/group/GroupInfoDisplayPolicy.kt`**
...
2. **`app/src/main/java/com/buyansong/im/group/GroupInfoViewModel.kt`**
...
3. **`app/src/main/java/com/buyansong/im/group/GroupInfoScreen.kt`**
   ...
   - **群聊名称卡片**（在网格下方，包裹在 `ByteImListSurface` 中）：
     ...
     ```
   - **改名 `AlertDialog`**（仅在 `state.showRenameDialog` 为真时渲染，与 [ChatScreen.kt:387-417](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L387) 形态完全一致）：`TextField(value = state.draftGroupName, onValueChange = ..., singleLine = true)`，confirmButton → `viewModel.confirmRename()`，dismissButton → `viewModel.cancelRename()`。保存期间 `confirmButton` 显示"保存中"且禁用。
   - 入口签名：
     ```kotlin
     @Composable
     fun GroupInfoScreen(
...
4. **`app/src/main/java/com/buyansong/im/SelfHostedImRoute.kt`**
...
5. **`app/src/main/java/com/buyansong/im/MainActivity.kt`**
   - 导入 `GroupInfoScreen` / `GroupInfoViewModel` / `GroupInfoDisplayPolicy`。
   - 在 `NavHost` 中新增 `composable(route = SelfHostedImRoute.GroupInfo.pattern) { entry -> ... }` 块（参考 [MainActivity.kt:580-606](../../app/src/main/java/com/buyansong/im/MainActivity.kt#L580) `ContactProfile` 的写法）：
   ...
   - 修改 `composable(SelfHostedImRoute.Chat.pattern)` 块（[MainActivity.kt:608-643](../../app/src/main/java/com/buyansong/im/MainActivity.kt#L608)）：在 `ChatScreen(...)` 调用上新增参数 `onOpenGroupInfo = { gid -> SelfHostedImRoute.GroupInfo.createRoute(gid)?.let(navController::navigate) }`。

6. **`app/src/main/java/com/buyansong/im/chat/ChatScreen.kt`**
   - 顶部 action 块（[ChatScreen.kt:201-218](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L201)）：
   ...
   - **删除** `var showGroupRename by remember { mutableStateOf(false) }`（[ChatScreen.kt:103](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L103)）、`var groupNameDraft by remember { mutableStateOf(state.peerName) }`（[ChatScreen.kt:108](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L108)）、`LaunchedEffect(state.peerName) { ... }`（[ChatScreen.kt:172-176](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L172)）、以及整段 `if (showGroupRename) { AlertDialog(...) }`（[ChatScreen.kt:387-417](../../app/src/main/java/com/buyansong/im/chat/ChatScreen.kt#L387)）。`ChatViewModel.renameGroup(...)` 保留不动（仍可被 `GroupInfoViewModel.confirmRename` 之外的其他场景复用，未来如要做"群主在成员页改名"也省得再接一次）。
