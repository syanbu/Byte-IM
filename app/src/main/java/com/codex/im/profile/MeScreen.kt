...
private fun ProfileGenderEditorScreen(
    state: MeUiState,
    onBack: () -> Unit,
    onSelectGender: (com.codex.im.storage.Gender) -> Unit,
    onDone: () -> Unit
) {
    ...
    Column(
        ...
        ByteImListSurface {
            GenderOptionRow(
                ...
                selected = draft == com.codex.im.storage.Gender.MALE,
                onClick = { onSelectGender(com.codex.im.storage.Gender.MALE) }
            )
            HorizontalDivider(color = ByteImColors.Divider)
            GenderOptionRow(
                label = MeDisplayPolicy.genderFemaleLabel,
                selected = draft == com.codex.im.storage.Gender.FEMALE,
                onClick = { onSelectGender(com.codex.im.storage.Gender.FEMALE) }
            )
        }
        state.errorMessage?.let { message ->
            Text(
