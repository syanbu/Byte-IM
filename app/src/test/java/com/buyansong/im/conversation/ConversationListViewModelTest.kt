...
class ConversationListViewModelTest {
    ...
    private class FakeGroupRepository(
        ...
        var conversationDao: com.buyansong.im.storage.ConversationDao?
    ) : GroupRepository {
        ...
        override suspend fun syncMembers(accessToken: String, groupId: String): List<com.buyansong.im.storage.GroupMember> {
            return emptyList()
        }

        override fun localMembers(groupId: String): List<com.buyansong.im.storage.GroupMember> {
            return emptyList()
        }
    ...
    private class CountingConversationDao(
        private val delegate: com.buyansong.im.storage.ConversationDao
    ) : com.buyansong.im.storage.ConversationDao by delegate {
        var pageQueryCount: Int = 0
        override fun listConversationsPage(
            beforeLastMessageTime: Long?,
            beforeConversationId: String?,
            limit: Int
        ): List<com.buyansong.im.storage.Conversation> {
            pageQueryCount++
